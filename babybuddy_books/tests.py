...
def _make_reading(child, book, **kwargs):
    defaults = dict(date_read=date(2024, 6, 1))
    defaults.update(kwargs)
...
class BookModelTestCase(TestCase):
    ...
    def test_default_ordering_is_by_title(self):
        child = _make_child()
        b2 = _make_book(title="Zebra Book")
        b1 = _make_book(title="Aardvark Book")
...
class BookReadingModelTestCase(TestCase):
    ...
    def test_str(self):
        reading = BookReading(
            child=self.child, book=self.book, date_read=date(2024, 3, 15)
        )
        self.assertIn(self.book.title, str(reading))
        self.assertIn("2024-03-15", str(reading))

    def test_default_ordering_newest_first(self):
        r1 = _make_reading(self.child, self.book, date_read=date(2024, 1, 1))
        r2 = _make_reading(self.child, self.book, date_read=date(2024, 6, 1))
...
class BookCardDataTestCase(TestCase):
    ...
    def test_count_reflects_readings(self):
        _make_reading(self.child, self.book)
        _make_reading(self.child, self.book, date_read=date(2024, 7, 1))
        data = book_card_data(self.child)
        self.assertEqual(data["count"], 2)

    def test_last_reading_is_most_recent(self):
        _make_reading(self.child, self.book, date_read=date(2024, 1, 1))
        recent = _make_reading(self.child, self.book, date_read=date(2024, 9, 1))
        data = book_card_data(self.child)
