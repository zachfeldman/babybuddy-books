...
class BookReadingAdd(PermissionRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        pending = request.POST.get("books_pending_data", "").strip()
        if pending and not request.POST.get("book"):
            try:
            ...
            if data.get("title"):
                ...
                book = (
                    Book.objects.filter(isbn=isbn).first()
                    if isbn
                    else None
                ) or Book.objects.create(
                ...
                post["book"] = str(book.pk)
                request.POST = post
        return super().post(request, *args, **kwargs)

    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        timer_id = self.request.GET.get("timer")
        if timer_id:
            kwargs["timer"] = timer_id
        return kwargs
